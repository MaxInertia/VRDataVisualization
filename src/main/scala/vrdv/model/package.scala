...
package object model {
  ...
  trait Room {
    ...
    def addRoom(scene: Scene): Unit = {
      ...
      val floorGeometry: PlaneGeometry = new PlaneGeometry( 4, 4 ) // was width = 1, height = 1
      val floor: Mesh = new Mesh(floorGeometry, floorMaterial)
      floor.receiveShadow = false
      floor.rotateX(-3.1415/2)

      // Add 6x6m Grid broken into 36 sections
      val floorGrid: GridHelper = new GridHelperExt(4, 8, black, black) // was size = 1, step = 2
